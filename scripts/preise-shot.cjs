// Preise-Builder Verify-Shot (Runde 1, 2026-08-07). Analog scripts/home-shot.cjs.
// Aufruf: node scripts/preise-shot.cjs <tag>
const { chromium } = require('/usr/lib/node_modules/playwright');
const fs = require('fs');

const BASE = 'http://localhost:5173';
const tag = process.argv[2] || 'base';
const OUT = `/tmp/sf-preise/${tag}`;
...
(async () => {
  ...
  for (const [name, w, h] of [
    ...
    // Horizontaler Overflow (Runde-3-Regel): body darf nicht breiter als der Viewport sein.
    const ow = await page.evaluate(() => document.documentElement.scrollWidth);
    console.log(name, 'scrollWidth', ow, 'viewport', w);
